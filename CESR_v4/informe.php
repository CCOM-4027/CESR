...
// QUERY TO RETRIVE ALL THE INFOMATION OF TIMESHEETS
	$info_query =  "SELECT WT.date, T.taskcode, T.t_name, WT.comment, WT.hours, Ph.ph_name
				   	FROM Worktime AS WT
					...
					WHERE WT.w_id = $worker AND WT.p_id = '$project' and MONTH(WT.date) = $month and YEAR(WT.date) = $year
   		...
   		// $worktime[] = $row['date'].";".$row['taskcode'].";".$row['comment'].";".$row['hours'].";".$row['ph_name']; 
   		 if ($row['taskcode'] == "SC"){
   		 		if (isset($_POST['external'])){
   		 			...
   		 			$S_entry[] = $row['date'].";".$row['taskcode'].";".$row['t_name'].";".$row['hours'].";".$row['ph_name'];
   				$SC+= $row['hours'];
   				}
   		 if ($row['taskcode'] == "AC"){
   		 		if (isset($_POST['external'])){
   		 			$A_entry[]= $row['date'].";".$row['taskcode'].";".$row['t_name'].";".$row['hours'].";".$row['ph_name'];
   		 		}
   		 		else
...
// CREATE PDF DOCUMENT
	...
	if (isset($_POST['external']))
		...
		$col= 'Comments';
	$header = array('Date', 'Activity', "$col", 'Hours', 'Phase');
